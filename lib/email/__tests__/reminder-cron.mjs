...
const REMINDER_WINDOW_MIN_HOURS = 12; // élargi de 22→12 pour absorber les gaps cron
...
// Hors fenêtre par le bas (10h avant = encore trop proche, < 12h)
test("OPT-A. Réservation dans 10h — hors fenêtre (< 12h, trop proche)", () => {
  const in10h = new Date(NOW.getTime() + 10 * 60 * 60 * 1000);
  const b = makeBooking({ starts_at: in10h.toISOString() });
  assert.equal(shouldSelectForReminder(b, NOW), false);
});

// Rattrapage — booking dans 15h (dans la nouvelle fenêtre [12h, 26h])
test("OPT-A2. Réservation dans 15h — rattrapée par la fenêtre élargie (+12h)", () => {
  // Scénario : le run d'il y a 9h a échoué. Le booking était alors à +24h.
  // Avec REMINDER_WINDOW_MIN_HOURS = 12, 15h est dans la fenêtre → rattrapé.
  const in15h = new Date(NOW.getTime() + 15 * 60 * 60 * 1000);
  const b = makeBooking({ starts_at: in15h.toISOString() });
  assert.equal(shouldSelectForReminder(b, NOW), true,
    "Un booking à +15h doit être rattrapé par la nouvelle fenêtre [+12h, +26h]");
});
