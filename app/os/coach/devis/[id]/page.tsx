...
export default async function DevisDetailPage({ params }: { params: Params }) {
  ...
  const issuedDate = new Date(quote.issued_at).toLocaleDateString("fr-FR", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  const expiresDate = new Date(quote.expires_at).toLocaleDateString("fr-FR", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span
              className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${QUOTE_STATUS_COLORS[quote.status]}`}
            >
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-taupe-300/40 bg-white p-5">
            ...
            <div className="space-y-2">
              {(["brouillon", "envoyé", "accepté", "refusé", "expiré"] as const).map(
                (status) => (
          ...
          <div className="rounded-2xl border border-red-100 bg-white p-5">
            ...
            <form action={deleteQuoteAction}>
              ...
              <button
                ...
                onClick={(e) => {
                  if (!confirm("Supprimer ce devis ?")) e.preventDefault();
                }}
              >
                Supprimer le devis
              </button>
