...
export function DeleteClientButton({
  ...
  async function handleDelete() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/os/clients/${clientId}`, { method: "DELETE" });
  ...
  return (
    <>
      {/* Bouton corbeille — visible au hover de la carte (group) */}
      <button
        type="button"
        onClick={openModal}
        className="absolute right-3 top-3 z-10 flex h-7 w-7 items-center justify-center rounded-lg text-taupe-300 opacity-0 transition-all group-hover:opacity-100 hover:bg-red-50 hover:!text-red-500"
          ...
          >
            {done ? (
              <div className="py-4 text-center">
                ...
                <p className="font-medium text-ink-900">Client supprimé</p>
                <p className="mt-1 text-sm text-taupe-500">Redirection en cours…</p>
              </div>
            ) : (
              <>
                ...
                <p className="mt-2 text-sm text-taupe-600">
                  ...
                  supprimées&nbsp;: séances, notes, mesures, bilans, objectifs et packs.
                </p>
