...
export default async function CoachSettingsPage({
  ...
  // Récupérer l'email de notification depuis le contenu coach
  const coaches = await loadCoaches();
  const coachContent = coaches.find((c) => c.osProfileId === profile.id);
  const notificationEmail =
    coachContent?.notification_email?.trim() ||
    coachContent?.email?.trim() ||
    null;
      ...
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        <div className="rounded-3xl border border-taupe-300/40 bg-white p-8 shadow-sm">
          <div className="mb-5">
            ...
            <p className="mt-2 text-sm text-taupe-600">
              Adresse où vous recevez les notifications de réservation.
            </p>
          </div>
          ...
          {notificationEmail ? (
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-ink-900">{notificationEmail}</p>
                <p className="mt-0.5 text-xs text-taupe-500">
                  Configuré par l'administrateur. Pour modifier, contactez Manon.
                </p>
              </div>
            </div>
          ) : (
            <div className="rounded-2xl border border-amber-200/60 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Aucun email de notification configuré. Contactez l'administrateur pour le renseigner.
            </div>
          )}
        </div>
              ...
              {gcalToken?.connected_at && (
                <p className="text-xs text-taupe-500">
                  ...
                  {new Intl.DateTimeFormat("fr-FR", {
                    ...
                    month: "long",
                    year: "numeric",
                  }).format(new Date(gcalToken.connected_at))}
                </p>
              )}
